...
Autonome mobile Roboter ermöglichen die Automatisierung von Aufgaben und Verhalten, die sich nicht nur
in Software abbilden lassen, sondern auch in die physikalische Welt durchschlagen. Für das Spiel
Tik-Tak-Toe zwischen Mensch und Roboter gibt es hierbei noch keine allgemeingültige Lösung.

In diesem Research Paper wird eine Lösung durch einen zweidimensionalen Plotter beleuchtet, basierend
auf der Kombination von Fischertechnik, Elektronik aus den Bauteilen des Roboter Kits von
Freenove und einem ESP32-WROVER. Hierzu werden zwei Stromkreise mit \qty{5}{V} und \qty{9}{V}
aufgebaut, die drei Motoren mit Hilfe von Motortreibern vom Typ L293D ansteuern. Diese ermöglichen somit
eine Positionierung eines Stiftes auf einem Blatt Papier inklusive Anheben und Absenken. Gesteuert wird
alles über den ESP32-WROVER, der eine Mensch-Maschinen-Interaktion über
ein Keypad zur Verfügung stellt. Abgerundet wird die vorgestellte Lösung durch ein in C implementierter
Algorithmus, der das Spielgeschehen steuert, das Spiel durch den Stift zeichnet und als Spielgegner
auftreten kann.
...
Im Laufe des Projektes sollte ein kleiner Roboter entwickelt werden, der Tik-Tak-Toe auf einem Blatt Papier gegen den Nutzer spielen kann. Die Wahl fiel auf Tik-Tak-Toe, weil es ein simples aber sehr bekanntes Spiel ist.

Das Team hat sich gegen die Nutzung eines schon existierenden Roboters oder Roboter-Kits
entschieden, um mit allen Bereichen der Robotik in Kontakt zu kommen. Ziel war es hier
eine fachschaftsübergreifende Kooperation zu nutzen, um einen optimalen Wissenstransfer
zu erreichen.
...
konstruiert. Die grundsätzlichen Anforderungen an die Hardware waren, dass sie eine Bewegung in alle drei Raumrichtungen
...
Der zweite Teil des Schlittens besteht aus einer Konstruktion, die verhindern soll, dass der Stift und der Mechanismus an dem er sich befindet, zu sehr verbiegen. Die L-förmige
...
Das Kegelzahnrad an der Schnecke wird durch einen XS Motor von Fischertechnik bewegt. Dieser befindet sich ebenfalls auf dem Schlitten. Das besondere an der Anordnung dieses Motors ist, dass er sich im rechten Winkel zur Bewegungsrichtung befindet. Die Momentübertragung kann in diesem Fall über Kegelzahnräder erfolgen, da diese eine konische Form besitzen.


\section{Elektronik}
Der Aufbau der Elektronik sowie die Verbindung mit dem Micro-Controller basieren auf den Roboter Kit von Freenove. 
Das hierbei verwendete Starter Kit beinhaltet neben dem ESP32-WROVER auch die nötigen Bauteile um die Schaltung aufbauen zu können. 
Dies war eine Entscheidungsgrundlage für die Verwendung des Robot-Kit\footnote{\url{https://github.com/Freenove/Freenove\_Ultimate\_Starter\_Kit}}.

Für die Verbindung zwischen dem System, dem ESP32-WROVER sowie der Fischertechnik, wurden die Verbindungskabel beider Systeme aufgetrennt und miteinander verbunden.
...
Für den Aufbau der Schaltung wird ein Breadboard verwendet, welches in Abbildung \ref{Elektrik:DevBoard} dargestellt ist. Die Verbindungen zwischen den darauf platzierten Bauteilen werden durch Steckbrückenkabel hergestellt, welche in das Board gesteckt und durch Klemmung fixiert werden.
...
dass die einzelnen Pins des Micro-Controllers mit bestimmten Leiterbahnen auf dem Breadboard verbunden werden.
...
\noindent Für die Gleichstrom-Elektromotoren werden Motoren aus dem Fischertechnik Sortiment verwendet. 
Diese Motoren sind aufgrund der Produktzugehörigkeit mit den passenden Aufnahmen versehen, um Kompatibilitätsprobleme mit der Mechanik auszuschließen.
...
\noindent Da beide Motoren mit jeweils \qty{9}{V} Gleichstrom (DC) betrieben werden, können sie nicht direkt über die Spannungsschiene des Micro-Controllers gespeist werden, 
...
wird eine \qty{9}{V} Spannungsversorgung angestrebt. Eine Richtungsänderung der Motoren kann durch das Tauschen der anliegenden Spannung erreicht werden.  
...
sondern das Signal periodisch zwischen den Zuständen \glqq An\grqq{} und \glqq Aus\grqq{} gewechselt. Die Drehgeschwindigkeit wird durch die Anpassung der Pulsweiten innerhalb der Perioden 
bestimmt \autocite{611797}. Aufgrund der hohen Frequenz wird eine konstante Drehbewegung erreicht. 
...
Alle Motoren benötigen unabhängig deren Leistungsdaten einen eigenen Treiber, damit jeder Motor in beide Richtungen angesteuert werden kann.

\subsection{Servo-Motor}

Alternative könnte ein Servo-Motor für die $z$-Achse verwendet werden, der die Höhe des Stiftes über einen Arm verändert. 
...
Für die Kommunikation mit dem Benutzer wird ein 4x4 Martrix Keypad verwendet. Die Verbindung zwischen Keypad und Micro-Controllers wird über das Breadboard hergestellt. 
Die nötigen 8 Pins werden in die entsprechenden Leiterbahnen des Breadboard gesteckt und werden durch das Extension Board  direkt auf den Pin des Micro-Controllers verbunden.
...
Wie in Abbildung \ref{Elektrik:Keypad} zu sehen ist, ist das Keypad mit jeweils 4 Spalten und Zeilen aufgebaut. Diese sind entsprechend mit den jeweiligen Tasten als Schalter verbunden. 
...
Durch die Verwendung des Keypads konnte die gesamte Interaktion für die Bedienung des Roboter auf dieser Schnittstelle abgebildet werden.
...
war, dass die Steuerung in der Lage ist, die einzelnen Motoren und weitere Hardware Komponenten
...
ist hier, dass man auch ein vollfunktionsfähiges Betriebssystem mitsamt allen dafür existierenden
...
eine dauerhafte Verbindung zu dem Computer. Dies ist nicht wünschenswert, da der Roboter autonom
...
Kommunikation mit dem Mobilegerät besitzen muss. Während des Baus des Prototyps ist es
...
auf dem jeweiligen Feld aktiv ist. Damit sind 16\,Bit Integer die kleinstmögliche Speichereinheit, in der
...
Um die Möglichkeit zu gewährleisten, zu unterschiedlichen Zeitpunkten an dem Roboter arbeiten
...
Wählt ein Spieler einen Zug aus, der nicht mehr spielbar ist, so erkennt die Spielelogik das und führt die entsprechende Bewegungsfunktion aus, um dies zu kommunizieren. Dies sorgt für eine
...
Der oben aufgeführte erste Teil beinhaltet Funktionen, deren Aufgabe alle Nutzereingaben abdecken.
...
Im aufgeführten zweiten Teil, sind alle Funktionen beinhaltet, die Bewegungen des Roboter darstellen. Diese haben den Zweck, zu einem bestimmten Spielfeld und wieder zurück zu kommen. Zu beachten ist hier,
...
Zur Navigation des Roboters wird auf allen Achsen auf Zeitangaben gesetzt, in denen die Motoren aktiviert werden. Die
...
Zuletzt benötigt der Roboter eine Möglichkeit zu kommunizieren, dass ein Spielzug nicht gespielt werden kann,
...
sichergestellt und erlaubte eine testbasierte Entwicklung auf einem leistungsstarken Computer ohne direkten
Hardwarezugriff zu benötigen.

\section{Fazit und Ausblick}

Dieser Abschnitt gibt einen Überblick über die offenen Probleme der Lösung, deren positiver 
...
des $x$-Motors ist, dass die
Antriebswelle nicht fixiert werden kann und sich schräg stellt, bevor das Abtriebsrad anfängt, sich zu drehen. Eine Fixierung der Welle hätte zur Reibung der Kette an den Bausteinen geführt.
...
Tinte zu hinterlassen. Zwar konnte der benötigte Druck durch Einsatz eines Filzstifts statt eines Kugelschreibers reduziert werden,
...
Einen zusätzlichen Verbesserungspunkt stellen die Motoren dar, da sie nicht in beide Richtungen exakt gleich schnell drehen. Dieses Problem konnte nicht durch eine Kalibrierung gelöst werden, da der Geschwindigkeitsunterschied abhängig von der Leistung der Stromversorgung ist. Eine
...
auf Grund mangelnder Anzahl an Pins am Chip und späten Auftretens im Projekt nicht genutzt. Zusätzlich
hätte die Software nicht mehr mit blockierenden Code arbeiten können, da man auf die Endschalter im Code
...
Spiellogik ist nicht sehr leistungshungrig, sodass noch mehr Funktionalitäten mit dem Roboter abgebildet werden könnten.
...
Chips, aber diese hätten immer ein langsames Aufspielen vorausgesetzt, was die Entwicklung verlangsamt hätte. Auch konnten so mehrere
Teammitglieder parallel und unabhängig voneinander sowohl an der Software als auch an der Hardware iterieren. Gleichzeitig war die
...
gesteigert werden kann, können auch Spiele implementiert werden, die mit kleineren Feldern auskommen müssen. Dies könnte beispielsweise
