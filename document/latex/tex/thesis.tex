...
Im Rahmen der Veranstaltung autonome mobile Roboter war das Ziel einen Roboter zu konstruieren, der in der Lage ist Tik-Tak-Toe zu spielen. Dieser Bericht beschreibt, wie die Mechanik, Elektronik und die Software erstellt wurden. 
Die Herausforderung bestand vor allem darin, die Schnittstellen zwischen den einzelnen Komponenten zu realisieren. Die Mechanik besteht aus Fischertechnik. Daraus wurde ein Plotter gebaut, der Bewegungen in alle drei Raumrichtungen unabhängig voneinander erlaubt. Währenddessen besteht die Elektronik aus den Bauteilen des Roboter Kits von Freenove mit dem ESP32-WROVER. Damit die Motoren an das Breadboard von Freenove angeschlossen werden konnten, mussten bestehende Kabel von beiden Systemen mithilfe von Stoßverbinder fest verbunden werden. Dabei kam zum einen das Problem auf, dass das Board nur maximal \qty{5}{V} Versorgungsspannung liefern kann, während die Motoren \qty{9}{V} benötigen. Zum anderen musste eine Lösung gefunden werden die Motoren in beide Richtungen drehen zu lassen. Beide Probleme konnten gelöst werden indem drei Motortreiber, L293D,  in die Schaltung eingebaut wurden. Diese erlauben zum einen die Bestimmung der Drehrichtung und zum anderen konnte so ein separater Stromkreis aufgebaut werden, der die geforderten \qty{9}{V} an die Motoren liefert. Als nächstes musste die Kommunikation mit dem Nutzer realisiert werden. Dies gelingt über ein Keypad. Jedem der neun möglichen Feldern beim Tik-Tak-Toe wurde eine Zahl zwischen eins und neun zugeordnet. Durch das Drücken der entsprechenden Taste fährt der Roboter in die gewünschte Position. Der Algorithmus selbst wurde in C implementiert und auf den ESP32-WROOM eingebettet. 
